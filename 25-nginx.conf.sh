...
include /etc/nginx/oidc-env/env.conf;
...
mkdir -p /etc/nginx/oidc-env

# Generate env directives in a separate file
cat > /etc/nginx/oidc-env/env.conf << 'ENVEOF'
# Auto-generated environment variables for NJS
# This file is generated by /docker-entrypoint.d/25-nginx.conf.sh
ENVEOF

env | sed -n '/^OIDC_/s/\([^=]*\)=.*$/env \1;/p' >> /etc/nginx/oidc-env/env.conf
